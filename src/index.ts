...
export function readDirectory(d64Binary: Uint8Array): DirectoryEntry[] {
    ...
    while (deTrack != 0) {
        ...
        for (var i = 0; i < 8; i++, offs += 32) {
            var fileType: FileType | undefined=undefined;
            ...
            var petsciiName = d64Binary.slice(offs + 0x05, offs + 0x15);
            dirEntries.push({
                type: fileType,
...
export function deleteDirectoryEntry(d64Binary: Uint8Array, d64FileOffset: any)
{
    let blankFile = new Uint8Array(0x20);
    blankFile.fill(0x00);
    d64Binary.set(blankFile, d64FileOffset);

}
export function writeDirectoryHeader(d64Binary: Uint8Array, header:any, id:any)
{
    //"  header title  ","XX ,["Disk ID", "X", "DOS VER"]
    //"0123456789ABCDEF","XX","ID","X","DS"
    let blankHeader = new Uint8Array(16);
    let blankHeaderID = new Uint8Array(5);
    const headerNameFileOffset = 0x16590; // $AA
    const headerIDFileOffset = 0x165a2; // $AA
